...
- Read `game/design/gdd/game-concept.md` if it exists â€?extract genre, scope, platform
...
**Question 1 â€?Prior experience** (ask this first, always, via `question`):
- Prompt: "Have you worked in any of these engines before?"
- Options: `Godot` / `Unity` / `Unreal Engine 5` / `SFML 3 (C++ library)` / `Raylib (C/C++ library)` / `Multiple â€?I'll explain` / `None of them`
...
**Question 2 â€?Target platform** (ask this second, always, via `question` â€?platform eliminates or heavily weights engines before any other factor):
...
- Platform rules that feed directly into the recommendation:
  - Mobile â†?Unity strongly preferred; Unreal is a poor fit; Godot is viable for simple mobile; SFML3 and Raylib require native compilation per platform (Android NDK, Emscripten for web) â€?significant extra effort
  - Console â†?Unity or Unreal; Godot and C++ library approaches require third-party publishers or significant porting work
  - Web â†?Godot exports cleanly to web; Unity WebGL is functional; Unreal has poor web support; Raylib has Emscripten support; SFML3 has no built-in web target
  - PC only â†?all engines viable including SFML3 and Raylib (their native home); other factors decide
  - Multiple â†?Unity is the most portable across PC/mobile/console; SFML3/Raylib require per-platform build configuration
...
- Real limitations: No visual editor â€?everything is code; no built-in physics, UI system, or scene graph; no asset pipeline; desktop-only (no mobile/web support); minimal community compared to Godot/Unity; you must build your own tooling; steeper initial setup time
- Licensing reality: zlib/libpng license â€?completely free with no restrictions whatsoever
...
**Raylib**
- Genuine strengths: Extremely simple API â€?designed for learning and rapid prototyping; supports many platforms (Windows, macOS, Linux, Web via Emscripten, Android, Raspberry Pi); raygui for immediate-mode UI; raymath for math helpers; active community; consistent cross-platform API surface; very small compiled binary
- Real limitations: No visual editor â€?everything is code; no built-in physics or scene graph; C API limits abstraction (no RAII, no namespaces in C) â€?C++ wrappers exist but are community-maintained; less suitable for large/complex games without significant scaffolding; limited 3D rendering compared to engines with deferred rendering
- Licensing reality: zlib/libpng license â€?completely free with no restrictions whatsoever
...
- 2D any style â†?Godot strongly preferred; Raylib viable for simple 2D; SFML3 excellent for custom-rendered 2D
- 3D stylized / atmospheric / contained world â†?Godot viable, Unity solid alternative; Raylib viable for simple 3D; SFML3 limited 3D (no built-in model loader)
...
- Mobile-first â†?Unity strongly preferred; Raylib has Android/iOS support but significant extra work
- Console-first â†?Unity or Unreal; C++ libraries require extensive porting
- Web â†?Godot; Raylib via Emscripten works for simple games
...
- Platformer 2D â†?Godot; SFML3 and Raylib both excellent for 2D with full control
- Strategy / top-down / RTS â†?Godot or Unity depending on 2D vs 3D; SFML3 excellent for 2D strategy
- Game jam / prototype â†?Raylib for speed; Godot if you want an editor
...
Use `question` with concept-specific deep-dive topics. Always generate these options from the user's actual concept â€?do not use generic options. Always include at minimum:
...
>
>   **A) C++** â€?RAII patterns, classes, STL, stronger type safety. Recommended for larger projects.
>   **B) C (Raylib only)** â€?Plain C API, simpler compilation, C11 standard. Best for small projects or learning.
>
> Which will this project primarily use?"
...
- Functions: PascalCase or snake_case â€?project preference, be consistent
...
- Headers: `.hpp` or `.h` â€?project preference, be consistent
- Constants: UPPER_SNAKE_CASE (e.g., `MAX_PLAYER_SPEED`)
- Member variables: `m_` prefix (e.g., `m_health`, `m_position`) â€?standard C++ practice
...
- Files: PascalCase or snake_case â€?project preference, be consistent
...
- **Language/Code Specialist**: sfml-specialist (C++ â€?single specialist covers all code)
- **Shader Specialist**: sfml-specialist (sf::Shader, GLSL integration)
- **UI Specialist**: sfml-specialist (no dedicated UI specialist â€?build UI with sf::Drawable or integrate Dear ImGui)
...
- **Language/Code Specialist**: raylib-specialist (C/C++ â€?single specialist covers all code)
- **Shader Specialist**: raylib-specialist (LoadShader, GLSL integration, rlgl raw OpenGL)
- **UI Specialist**: raylib-specialist (raygui header â€?immediate-mode GUI)
- **Additional Specialists**: None
...
### 7.3. Configure godot-mcp (Optional â€?Godot Only)
...
**Available MCP tools:**
- `launch_editor` â€?launch the Godot editor
- `run_project` â€?run the current project
- `get_debug_output` â€?capture live debug output from the running project
- `stop_project` â€?stop the running project
- `get_godot_version` â€?check the installed Godot version
- `list_projects` â€?list all Godot projects
- `get_project_info` â€?get metadata about the project
- `create_scene` â€?create a new scene file
- `add_node` â€?add nodes to a scene
- `load_sprite` â€?load a sprite resource
- `save_scene` â€?save a scene file
- `export_mesh_library` â€?export a mesh library
- `get_uid` â€?get a resource UID
- `update_project_uids` â€?update project resource UIDs
...
### 7.4. Build & Run Setup (SFML 3 / Raylib â€?No MCP Available)
...
```
project-root/
â”œâ”€â”€ CMakeLists.txt          # cmake_minimum_required, project(), add_executable, target_link_libraries
â”œâ”€â”€ src/                    # Game source code
â”œâ”€â”€ assets/                 # Textures, sounds, fonts, shaders
...
- NEVER guess an engine version â€?always verify via webfetch or user confirmation
