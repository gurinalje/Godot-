...
1. **Cross-GDD Consistency** ‚Ä?contradictions, stale references, and ownership
   conflicts between documents
2. **Game Design Holism** ‚Ä?issues that only emerge when you see all systems
...
### Phase 1a ‚Ä?L0: Summary Scan (fast, low tokens)
...
Found [N] GDDs. Summaries:
  ‚Ä?combat.md ‚Ä?[summary text]
  ‚Ä?inventory.md ‚Ä?[summary text]
  ...
```
...
### Phase 1b ‚Ä?Registry Pre-Load (fast baseline)
...
names first ‚Ä?this is faster than reading all GDDs in full before knowing
what to look for.

If the registry is empty or absent: proceed without it. Note in the report:
"Entity registry is empty ‚Ä?consistency checks rely on full GDD reads only.
Run `/consistency-check` after this review to populate the registry."

### Phase 1c ‚Ä?L1/L2: Full Document Load

Full-read the in-scope documents:

1. `game/design/gdd/game-concept.md` ‚Ä?game vision, core loop, MVP definition
2. `game/design/gdd/game-pillars.md` if it exists ‚Ä?design pillars and anti-pillars
3. `game/design/gdd/systems-index.md` ‚Ä?authoritative system list, layers, dependencies, status
4. **Every in-scope system GDD in `game/design/gdd/`** ‚Ä?read completely (skip
   game-concept.md and systems-index.md ‚Ä?those are read above)
...
Phase 2 (Consistency) and Phase 3 (Design Theory) are independent ‚Ä?they read
...
```
‚ö†Ô∏è  Dependency Asymmetry
[system-a].md lists: Depends On ‚Ü?[system-b].md
[system-b].md does NOT list [system-a].md as a dependent
‚Ü?One of these documents has a stale dependency section
```
...
‚Ü?These rules directly contradict. Which GDD is authoritative?
```
...
```
‚ö†Ô∏è  Stale Reference
inventory.md (written first): "Item weight uses the encumbrance formula
  from movement.md"
movement.md (written later): Defines no encumbrance formula ‚Ä?uses a flat
  carry limit instead
‚Ü?inventory.md references a formula that doesn't exist
```
...
```
‚ö†Ô∏è  Ownership Conflict
[system-a].md Tuning Knobs: "[multiplier_name] ‚Ä?controls [output] scaling"
[system-b].md Tuning Knobs: "[multiplier_name] ‚Ä?scales [output] with [factor]"
‚Ü?Two GDDs define multipliers on the same output. Which owns the final value?
...
- If [system-a].md outputs values between [min]‚Äì[max], and [system-b].md is
  designed to receive values between [min2]‚Äì[max2], is the mismatch intentional?
...
```
‚ö†Ô∏è  Competing Progression Loops
...
‚Ü?Three systems all claim to be the primary progression loop and all award
...
```
‚ö†Ô∏è  Cognitive Load Risk
Simultaneously active systems during [core loop moment]:
  1. [system-a].md ‚Ä?[decision type] (active)
  2. [system-b].md ‚Ä?[resource management] (active)
  3. [system-c].md ‚Ä?[tracking] (active)
  4. [system-d].md ‚Ä?[item/action use] (active)
  5. [system-e].md ‚Ä?[cooldown/timer management] (active)
  6. [system-f].md ‚Ä?[coordination decisions] (active)
‚Ü?6 simultaneously active systems during the core loop.
...
A dominant strategy makes other strategies irrelevant ‚Ä?players discover it,
...
```
‚ö†Ô∏è  Potential Dominant Strategy
...
‚Ü?Unless melee has a significant compensating advantage (AOE, stagger,
  resource regeneration), ranged is dominant ‚Ä?higher safety, only 20% less
...
| **Positive feedback loop** | More resource ‚Ü?easier to earn more | Runaway leader, snowball |
| **No catch-up** | Falling behind accelerates deficit | Unrecoverable states |

```
üî¥ Economic Imbalance: Unbounded Positive Feedback
gold economy:
...
‚Ü?After equipment and abilities are purchased, gold has no sink.
...
```
‚ö†Ô∏è  Difficulty Curve Mismatch
combat.md: Enemy health scales exponentially with area (√ó2 per area)
progression.md: Player damage scales linearly with level (+10% per level)
‚Ü?By area 5, enemies have 32√ó base health; player deals ~1.5√ó base damage.
...
serves no pillar is "scope creep by design" ‚Ä?it's in the game but not in
...
```
‚ö†Ô∏è  Pillar Drift
fishing-system.md: Player Fantasy ‚Ä?"peaceful, meditative activity"
Pillars: "Brutal Combat", "Tense Survival", "Emergent Stories"
‚Ü?The fishing system serves none of the three pillars. Either add a pillar
  that covers it, redesign it to serve an existing pillar, or cut it.
```

Also check anti-pillars ‚Ä?flag any system that does what an anti-pillar
explicitly says the game will NOT do:

```
üî¥ Anti-Pillar Violation
Anti-Pillar: "We will NOT have linear story progression ‚Ä?player defines their path"
main-quest.md: Defines a 12-chapter linear story with mandatory sequence
‚Ü?This system directly violates the defined anti-pillar.
```

### 3g: Player Fantasy Coherence

The player fantasies across all systems should be compatible ‚Ä?they should
...
```
‚ö†Ô∏è  Player Fantasy Conflict
combat.md: "You are a ruthless, precise warrior ‚Ä?every kill is earned"
dialogue.md: "You are a charismatic diplomat ‚Ä?violence is always avoidable"
exploration.md: "You are a reckless adventurer ‚Ä?diving in without a plan"
‚Ü?Three systems present incompatible identities. Players will feel the game
...
appear at the interaction boundary between multiple systems ‚Ä?things static
analysis of individual GDDs cannot surface.

### 4a: Identify Key Multi-System Moments

Scan all GDDs and identify the 3‚Ä? most important player-facing moments where
...
1. **Trigger** ‚Ä?what player action or game event starts this?
2. **Activation order** ‚Ä?which systems activate, in what sequence?
3. **Data flow** ‚Ä?what does each system output, and is that output a valid
   input for the next system in the chain?
4. **Player experience** ‚Ä?what does the player see, hear, or feel at each step?
5. **Failure modes** ‚Ä?are there any of the following?
...
‚Ü?combat.md: awards kill XP (100 pts)
‚Ü?progression.md: XP total crosses level threshold ‚Ü?triggers level-up
  Output: new level, stat increases, ability unlock popup
‚Ü?quest.md: kill-count criterion met ‚Ü?triggers quest completion event
  Output: quest reward XP (500 pts), completion fanfare
‚Ü?progression.md (again): quest XP added ‚Ü?triggers SECOND level-up in same frame
  ‚ö†Ô∏è  Data flow issue: quest.md awards XP without checking if a level-up
...
- **BLOCKER**: undefined behavior, broken state transition, or contradictory
  player messaging ‚Ä?the experience is broken or incoherent in this scenario
- **WARNING**: compounding spikes, feedback loops without caps, reward conflicts ‚Ä?  the experience works but produces unintended outcomes
- **INFO**: minor ordering ambiguity or messaging overlap ‚Ä?worth noting but
...
### If FAIL ‚Ä?required actions before re-running:
...
- Options: `[A] Yes ‚Ä?write the report` / `[B] No ‚Ä?skip`
...
- Options: `[A] Yes ‚Ä?update systems index` / `[B] No ‚Ä?leave as-is`
- If yes: update each flagged GDD's Status field in systems-index.md to "Needs Revision".
  (Do NOT append parentheticals to the status value ‚Ä?other skills match "Needs Revision"
...
append to `production/session-state/active.md`:
    ...
    ## Session Extract ‚Ä?/review-all-gdds [date]
    - Verdict: [PASS / CONCERNS / FAIL]
    - GDDs reviewed: [N]
    - Flagged for revision: [comma-separated list, or "None"]
    - Blocking issues: [N ‚Ä?brief one-line descriptions, or "None"]
...
- Are there any Warning-level items that are simple edits (flagged with "30-second edit", "brief addition", or similar)? ‚Ü?offer inline quick-fix option
- Are any GDDs in the "Flagged for Revision" table? ‚Ü?offer /design-review option for each
- Read systems-index.md for the next system with Status: Not Started ‚Ü?offer /design-system option
- Is the verdict PASS or CONCERNS? ‚Ü?offer /gate-check or /create-architecture

Build the option list dynamically ‚Ä?only include options that apply:

**Option pool:**
- `[_] Apply quick fix: [W-XX description] in [gdd-name].md ‚Ä?[effort estimate]` (one option per simple-edit warning; only for Warning-level, not Blocking)
- `[_] Run /design-review [flagged-gdd-path] ‚Ä?address flagged warnings` (one per flagged GDD, if any)
- `[_] Run /design-system [next-system] ‚Ä?next in design order` (always include, name the actual system)
- `[_] Run /create-architecture ‚Ä?begin architecture (verdict is PASS/CONCERNS)` (include if verdict is not FAIL)
- `[_] Run /gate-check ‚Ä?validate Systems Design phase gate` (include if verdict is PASS)
- `[_] Stop here`

Assign letters A, B, C‚Ä?only to included options. Mark the most pipeline-advancing option as `(recommended)`.
...
1. **Surface immediately**: Report "[AgentName]: BLOCKED ‚Ä?[reason]" before continuing
...
4. **Always produce a partial report** ‚Ä?output whatever was completed so work is not lost

---

## Collaborative Protocol

1. **Read silently** ‚Ä?load all GDDs before presenting anything
2. **Show everything** ‚Ä?present the full consistency and design theory analysis
   before asking for any action
3. **Distinguish blocking from advisory** ‚Ä?not every issue needs to block
   architecture; be clear about which do
4. **Don't make design decisions** ‚Ä?flag contradictions and options, but never
   unilaterally decide which GDD is "right"
5. **Ask before writing** ‚Ä?confirm before writing the report or updating the
   systems index
6. **Be specific** ‚Ä?every issue must cite the exact GDD, section, and text
