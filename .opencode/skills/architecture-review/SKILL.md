...
**Argument modes:**
- **No argument / `full`**: Full review ‚Ä?all phases
- **`coverage`**: Traceability only ‚Ä?which GDD requirements have no ADR
...
- **`rtm`**: Requirements Traceability Matrix ‚Ä?extends the standard matrix
  to include story file paths and test file paths; outputs
  `docs/architecture/requirements-traceability.md` with the full
  GDD requirement ‚Ü?ADR ‚Ü?Story ‚Ü?Test chain. Use in Production phase when
...
For `engine` mode: only full-read ADRs ‚Ä?GDDs are not needed for engine checks.

For `coverage` or `full` mode: proceed to full-read everything below.

### Phase 1b ‚Ä?L1/L2: Full Document Load

Read all inputs appropriate to the mode:

### Design Documents
- All in-scope GDDs in `game/design/gdd/` ‚Ä?read every file completely
- `game/design/gdd/systems-index.md` ‚Ä?the authoritative list of systems

### Architecture Documents
- All in-scope ADRs in `docs/architecture/` ‚Ä?read every file completely
...
1. **Exact/near match** to an existing registry entry for the same system ‚Ü?   reuse that entry's TR-ID unchanged. Update the `requirement` text in the
   registry only if the GDD wording changed (same intent, clearer phrasing) ‚Ä?   add a `revised: [date]` field.
2. **No match** ‚Ü?assign a new ID: next available `TR-[system]-NNN` for that
   system, starting from the highest existing sequence + 1.
3. **Ambiguous** (partial match, intent unclear) ‚Ü?ask the user:
...
For any requirement with `status: deprecated` in the registry ‚Ä?skip it.
It was removed from the GDD intentionally.

For each GDD, read it and extract all **technical requirements** ‚Ä?things the
...
| Category | Example |
|----------|---------|
| **Data structures** | "Each entity has health, max health, status effects" ‚Ü?needs a component/data schema |
| **Performance constraints** | "Collision detection must run at 60fps with 200 entities" ‚Ü?physics budget ADR |
| **Engine capability** | "Inverse kinematics for character animation" ‚Ü?IK system ADR |
| **Cross-system communication** | "Damage system notifies UI and audio simultaneously" ‚Ü?event/signal architecture ADR |
| **State persistence** | "Player progress persists between sessions" ‚Ü?save system ADR |
| **Threading/timing** | "AI decisions happen off the main thread" ‚Ü?concurrency ADR |
| **Platform requirements** | "Supports keyboard, gamepad, touch" ‚Ü?input system ADR |
...
Technical Requirements:
  TR-[GDD]-001: [requirement text] ‚Ü?Domain: [Physics/Rendering/etc]
  TR-[GDD]-002: [requirement text] ‚Ü?Domain: [...]
```

This becomes the **requirements baseline** ‚Ä?the complete set of what the
...
| Status | Meaning |
|--------|---------|
| ‚ú?**Covered** | An ADR explicitly addresses this requirement |
| ‚ö†Ô∏è **Partial** | An ADR partially covers this, or coverage is ambiguous |
| ‚ù?**Gap** | No ADR addresses this requirement |
...
|---------------|-----|--------|-------------|--------------|--------|
| TR-combat-001 | combat.md | Combat | Hitbox detection < 1 frame | ADR-0003 | ‚ú?|
| TR-combat-002 | combat.md | Combat | Combo window timing | ‚Ä?| ‚ù?GAP |
| TR-inventory-001 | inventory.md | Inventory | Persistent item storage | ADR-0005 | ‚ú?|
```
...
each requirement and the test that verifies it ‚Ä?producing the full
Requirements Traceability Matrix (RTM).

### Step 3b-1 ‚Ä?Load stories
...
- Extract `## Test Evidence` section ‚Ä?the stated test file path

### Step 3b-2 ‚Ä?Load test files

Glob `game/tests/unit/**/*_test.*` and `game/tests/integration/**/*_test.*`.
Build an index: system ‚Ü?[test file paths].
...
### Step 3b-3 ‚Ä?Build the extended RTM
...
- **Test Status**: COVERED (test file exists) / MISSING (path stated but not
  ...
  NO STORY (requirement has no story yet ‚Ä?pre-production gap)
...
| TR-combat-002 | combat.md | Combo window | ‚Ä?| story-002-combo.md | ‚Ä?| NONE (Visual/Feel) |
| TR-inventory-001 | inventory.md | Persistent storage | ADR-0005 | ‚Ä?| ‚Ä?| NO STORY |
```

RTM coverage summary:
- COVERED: [N] ‚Ä?requirements with ADR + story + passing test
- MISSING test: [N] ‚Ä?story exists but test file not found
- NO STORY: [N] ‚Ä?requirements with ADR but no story yet
- NO ADR: [N] ‚Ä?requirements without architectural coverage (from Phase 3 gaps)
...
2. **Topological sort**: Determine the correct implementation order ‚Ä?ADRs with no
...
3. **Flag unresolved dependencies**: If ADR-A's "Depends On" field references an ADR
   ...
   ```
   ‚ö†Ô∏è  ADR-0005 depends on ADR-0002 ‚Ä?but ADR-0002 is still Proposed.
...
4. **Cycle detection**: If ADR-A depends on ADR-B and ADR-B depends on ADR-A (directly
   ...
   ```
   üî¥ DEPENDENCY CYCLE: ADR-0003 ‚Ü?ADR-0006 ‚Ü?ADR-0003
...
- These are blind spots ‚Ä?their engine assumptions are unknown
...
Deprecated API References:
  - ADR-0002: uses [deprecated API] ‚Ä?deprecated since [version]

Stale Version References:
  - ADR-0001: written for [older version] ‚Ä?current project version is [version]
...
- Spawn `subagent_type: [primary specialist]` with: all ADRs that contain engine-specific decisions or `Post-Cutoff APIs Used` fields, the engine reference docs, and the Phase 5 audit findings. Ask them to:
  1. Confirm or challenge each audit finding ‚Ä?specialists may know of engine nuances not captured in the reference docs
...
Incorporate additional findings under `### Engine Specialist Findings` in the Phase 5 output. These feed into the final verdict ‚Ä?specialist-identified issues carry the same weight as audit-identified issues.

---

## Phase 5b: Design Revision Flags (Architecture ‚Ü?GDD Feedback)
...
```
### GDD Revision Flags (Architecture ‚Ü?Design Feedback)
...
|-----|-----------|--------------------------------------|--------|
| combat.md | "Use HingeJoint3D damp for weapon recoil" | Jolt ignores damp ‚Ä?ADR-0003 | Revise GDD |
```

If no revision flags are found, write: "No GDD revision flags ‚Ä?all GDD assumptions
...
- If yes: update the relevant systems' Status field to "Needs Revision"
  ...
  (Do NOT use parentheticals like "Needs Revision (Architecture Feedback)" ‚Ä?other skills
...
For each gap:
  ‚ù?TR-[id]: [GDD] ‚Ü?[system] ‚Ü?[requirement]
...
[Topologically sorted implementation order from Phase 4 ‚Ä?dependency ordering section]
[Unresolved dependencies and cycles if any]

### GDD Revision Flags
[GDD assumptions that conflict with verified engine behaviour ‚Ä?from Phase 5b]
[Or: "None ‚Ä?all GDD assumptions consistent with verified engine behaviour"]
...
[List items that must be resolved ‚Ä?FAIL verdict only]
...
- "Review complete. What would you like to write?"
  ...
  - [B] Write review report only ‚Ä?`docs/architecture/architecture-review-[date].md`
  - [C] Don't write anything yet ‚Ä?I need to review the findings first
...
> Coverage: [N]% full chain complete (GDD ‚Ü?ADR ‚Ü?Story ‚Ü?Test)
...
| Status | Count | % |
|--------|-------|---|
| COVERED ‚Ä?full chain complete | [N] | [%] |
| MISSING test ‚Ä?story exists, no test | [N] | [%] |
| NO STORY ‚Ä?ADR exists, not yet implemented | [N] | [%] |
| NO ADR ‚Ä?architectural gap | [N] | [%] |
...
After writing the review report, append any üî¥ CONFLICT entries found in Phase 4
to `docs/consistency-failures.md` (if the file exists):

```markdown
### [YYYY-MM-DD] ‚Ä?/architecture-review ‚Ä?üî¥ CONFLICT
**Domain**: Architecture / [specific domain e.g. State Ownership, Performance]
**Documents involved**: [ADR-NNNN] vs [ADR-MMMM]
**What happened**: [specific conflict ‚Ä?what each ADR claims]
**Resolution**: [how it was or should be resolved]
**Pattern**: [generalised lesson for future ADR authors in this domain]
```

Only append CONFLICT entries ‚Ä?do not log GAP entries (missing ADRs are expected
before the architecture is complete). Do not create the file if missing ‚Ä?only
...
`production/session-state/active.md`:
    ...
    ## Session Extract ‚Ä?/architecture-review [date]
    - Verdict: [PASS / CONCERNS / FAIL]
    - Requirements: [N] total ‚Ä?[X] covered, [Y] partial, [Z] gaps
...
## Known Gaps
[All ‚ù?items with suggested ADRs]
...
- "Architecture review complete. What would you like to do next?"
  - [A] Write a missing ADR ‚Ä?open a fresh session and run `/architecture-decision [system]`
  - [B] Run `/gate-check pre-production` ‚Ä?if all blocking gaps are resolved
...
1. **Read silently** ‚Ä?do not narrate every file read
2. **Show the matrix** ‚Ä?present the full traceability matrix before asking for
   anything; let the user see the state
3. **Don't guess** ‚Ä?if a requirement is ambiguous, ask: "Is [X] a technical
   requirement or a design preference?"
4. **Ask before writing** ‚Ä?always confirm before writing the report file
5. **Non-blocking** ‚Ä?the verdict is advisory; the user decides whether to continue
