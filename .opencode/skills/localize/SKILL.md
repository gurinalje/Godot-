...
Localization is not just translation ‚Ä?it is the full process of making a game
...
**Modes:**
- `scan` ‚Ä?Find hardcoded strings and localization anti-patterns (read-only)
- `extract` ‚Ä?Extract strings and generate translation-ready tables
- `validate` ‚Ä?Check translations for completeness, placeholders, and length
- `status` ‚Ä?Coverage matrix across all locales
- `brief` ‚Ä?Generate translator context briefing document for an external team
- `cultural-review` ‚Ä?Flag culturally sensitive content, symbols, colours, idioms
- `vo-pipeline` ‚Ä?Manage voice-over localization: scripts, recording specs, integration
- `rtl-check` ‚Ä?Validate RTL language layout, mirroring, and font support
- `freeze` ‚Ä?Enforce string freeze; lock source strings before translation begins
- `qa` ‚Ä?Run the full localization QA cycle before release

If no subcommand is provided, output usage and stop. Verdict: **FAIL** ‚Ä?missing required subcommand.
...
- Hardcoded punctuation (e.g. `"You won!"` ‚Ä?exclamation styles vary by locale)

Report all findings with file paths and line numbers. This mode is read-only ‚Ä?no files are written.
...
- Each new entry must include a `context` field ‚Ä?a translator comment explaining:
...
If yes, write only the diff (new entries), not a full replacement. Verdict: **COMPLETE** ‚Ä?strings extracted and written.
...
- **Completeness** ‚Ä?key exists in source (en) but no translation for this locale
- **Placeholder mismatches** ‚Ä?source has `{name}` but translation omits it or adds extras
- **String length violations** ‚Ä?translation exceeds the character limit recorded in the source `context` field
- **Plural form count** ‚Ä?locale requires N plural forms; translation provides fewer
- **Orphaned keys** ‚Ä?translation exists but nothing in `src/` references the key
- **Stale translations** ‚Ä?source string changed after translation was written (flag for re-translation)
- **Encoding** ‚Ä?non-ASCII characters present and font atlas supports them (flag if uncertain)

Report validation results grouped by locale and severity. This mode is read-only ‚Ä?no files are written.
...
This mode is read-only ‚Ä?no files are written.
...
Read:
- `game/design/gdd/` ‚Ä?extract game genre, tone, setting, character names
- `assets/data/strings/strings-en.json` ‚Ä?the source string table
...
```markdown
# Translator Brief ‚Ä?[Game Name] ‚Ä?[Locale]

## Game Overview
[2-3 paragraph summary of the game, genre, tone, and audience]

## Tone and Voice
- **Overall tone**: [e.g., "Darkly comic, not slapstick ‚Ä?think Terry Pratchett, not Looney Tunes"]
- **Player address**: [e.g., "Second person, informal. Never formal 'vous' ‚Ä?always 'tu' for French"]
- **Profanity policy**: [e.g., "Mild ‚Ä?PG-13 equivalent. Match intensity to source, do not soften or escalate"]
- **Humour**: [e.g., "Wordplay exists ‚Ä?if a pun cannot translate, invent an equivalent local joke; do not translate literally"]
...
- Thumbs up, OK hand, peace sign ‚Ä?meanings vary by region
...
**Numbers**
- 4 (death in Japanese/Chinese), 13, 666 ‚Ä?flag use in UI (room numbers, item counts, prices)
...
- Blood colour, gore level, drug references ‚Ä?flag all for region-specific asset variants if needed
...
- `vo-pipeline scan` ‚Ä?identify all dialogue lines that require VO recording
- `vo-pipeline script` ‚Ä?generate recording scripts with director notes
- `vo-pipeline validate` ‚Ä?check that all recorded VO files are present and correctly named
- `vo-pipeline integrate` ‚Ä?verify VO files are correctly referenced in code/assets
...
- Any `HBoxContainer`, `LinearLayout`, `HorizontalBox` nodes ‚Ä?verify layout_direction settings
...
If already frozen:
> "String freeze is currently ACTIVE (called [date]). [N] strings have been added or modified since freeze. These are freeze violations ‚Ä?they require re-translation or an approved freeze lift."
...
- Options: `[A] Yes ‚Ä?call string freeze now` / `[B] No ‚Ä?I still have strings to add`
...
When `extract` mode finds new or modified strings and `freeze-status.md` shows Status: ACTIVE ‚Ä?append the new keys to `## Post-Freeze Changes` and warn:
> "‚ö†Ô∏è String freeze is active. [N] new/modified strings have been added. These are freeze violations. Notify your localization vendor before proceeding."
...
‚Ä?this is a structured playthrough-based quality check.

Spawn `localization-lead` via Task with:
- The target locale(s) to QA
- The list of all screens/flows in the game (from `game/design/gdd/` or `/content-audit` output)
...
1. **Functional string check** ‚Ä?every string displays in-game without truncation, placeholder errors, or encoding corruption
2. **UI overflow check** ‚Ä?translated strings that exceed UI bounds (even if within character limits, some languages expand)
3. **Contextual accuracy** ‚Ä?a sample of 10% of strings reviewed in-game for translation accuracy and natural phrasing
4. **Cultural review items** ‚Ä?verify all BLOCKING items from the cultural review are resolved
5. **VO sync check** ‚Ä?if VO exists, verify lip sync or subtitle timing is acceptable after translation
6. **Platform cert requirements** ‚Ä?check platform-specific localization requirements (age ratings text, legal notices, ESRB/PEGI/CERO text)

Output a QA verdict per locale:

```
## Localization QA Verdict ‚Ä?[Locale]
...
| LOC-002 | Translation | [Key] translation is literal ‚Ä?sounds unnatural | ADVISORY | Open |

### Conditions (if PASS WITH CONDITIONS)
- [Condition 1 ‚Ä?must resolve before ship]
...
**Gate integration**: The Polish ‚Ü?Release gate requires a PASS or PASS WITH CONDITIONS verdict for every locale being shipped. A FAIL blocks release for that locale only ‚Ä?other locales may still proceed if their QA passes.
...
- Never modify translation files directly ‚Ä?generate diffs for review
- Character limits must be defined per-UI-element and enforced in validate mode
- String freeze must be called before sending strings to translators ‚Ä?never translate a moving target
- RTL support must be designed in from the start ‚Ä?retrofitting RTL layout is expensive
- Cultural review is required for any locale where the game will be sold commercially
- VO scripts must include director notes ‚Ä?raw dialogue lines produce flat recordings

### Recommended Workflow

```
/localize scan            ‚Ü?find hardcoded strings
/localize extract         ‚Ü?build string table
/localize freeze          ‚Ü?lock source before sending to translators
/localize brief           ‚Ü?generate translator briefing document
[Send to translators]
/localize validate        ‚Ü?check returned translations
/localize cultural-review ‚Ü?flag culturally sensitive content
/localize rtl-check       ‚Ü?if shipping Arabic / Hebrew / Persian
/localize vo-pipeline     ‚Ü?if shipping dubbed VO
/localize qa              ‚Ü?full localization QA pass
```
