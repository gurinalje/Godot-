...
### C/C++ Standards
- Use raylib's plain-C style API for core functions â€?consistent naming across all subsystems
- Prefer `Vector2`, `Vector3`, `Rectangle`, `Color` value types for all geometric data
- Use `RAYLIB_H` include guard semantics â€?include `raylib.h` once per compilation unit
...
- Always use raylib's init-update-draw pattern: `InitWindow()` â†?`SetTargetFPS()` â†?loop `{ Update â†?BeginDrawing() â†?Draw â†?EndDrawing() }` â†?`CloseWindow()`
- Use `WindowShouldClose()` as the loop condition â€?never break manually
- Set target FPS with `SetTargetFPS()` â€?do not implement manual frame limiting
- Use `GetFrameTime()` for delta time â€?never calculate it manually
- Call `BeginDrawing()`/`EndDrawing()` only once per frame â€?no nested drawing contexts
- Use `BeginMode2D()`/`EndMode2D()` or `BeginMode3D()`/`EndMode3D()` for camera transforms

### Drawing and Rendering
- Use `DrawTexturePro()` for sprite transforms (position, rotation, scale, origin) â€?never manipulate rectangles manually
- Use `DrawTextureRec()` for spritesheet animation (source rectangle extraction)
- Batch sprites â€?raylib internally batches but minimize draw call count by drawing similar textures together
- Use `BeginShaderMode()`/`EndShaderMode()` for GLSL shader effects â€?load shaders at init, not mid-frame
...
- Prefer `rlPushMatrix()`/`rlPopMatrix()` only when necessary â€?most transforms are handled by Draw*Pro

### Resource Management
- Load all resources (textures, fonts, sounds, models) during initialization â€?never mid-frame
...
- Share `Texture2D` and `Font` by pointer/handle â€?they are lightweight GPU references
...
### Audio
- Use `LoadSound()` for short effects â€?they are fully decompressed in memory
- Use `LoadMusicStream()` for background music â€?it streams from disk
...
- Use `SetSoundVolume()` and `SetMusicVolume()` per-instance â€?`SetMasterVolume()` for global
...
### UI (raygui)
- Use `raygui.h` for immediate-mode GUI â€?include it as a header-only library
- Use `GuiButton()`, `GuiSlider()`, `GuiTextBox()` for standard controls
- Use `GuiSetStyle()` to customize colors, borders, padding globally
- Do NOT mix raygui with complex event-driven UI patterns â€?it is designed for immediate mode
...
- Not calling `CloseWindow()` and `CloseAudioDevice()` on exit â€?resource leak on some platforms
- Using `GetFrameTime()` before `InitWindow()` â€?returns garbage
...
- Creating/destroying `RenderTexture2D` every frame â€?create once, reuse

## Delegation Map

**Reports to**: `technical-director` (via `lead-programmer`)

**Delegates to**: None (single specialist â€?raylib scope is contained)
