...
3. **Scan design documents** in `game/design/gdd/` for features tagged as ready
   for implementation.
...
**Generate a sprint plan** following this format and present it to the user. Do NOT ask to write yet â€?the producer feasibility gate (Phase 4) runs first and may require revisions before the file is written.
...
This is the machine-readable source of truth for story status â€?read by
...
# DO NOT edit manually â€?use /story-done to update story status.
...
- Must Have tasks â†?`priority: must-have`, `status: ready-for-dev`
- Should Have tasks â†?`priority: should-have`, `status: backlog`
- Nice to Have tasks â†?`priority: nice-to-have`, `status: backlog`
...
**Review mode check** â€?apply before spawning PR-SPRINT:
- `solo` â†?skip. Note: "PR-SPRINT skipped â€?Solo mode." Proceed to Phase 5 (QA plan gate).
- `lean` â†?skip (not a PHASE-GATE). Note: "PR-SPRINT skipped â€?Lean mode." Proceed to Phase 5 (QA plan gate).
...
After handling the producer's verdict, ask: "May I write this sprint plan to `production/sprints/sprint-[N].md`?" If yes, write the file, creating the directory if needed. Verdict: **COMPLETE** â€?sprint plan created. If no: Verdict: **BLOCKED** â€?user declined write.
...
**If a QA plan is found**: note it in the sprint plan output â€?"QA Plan: `[path]`" â€?and proceed.

**If no QA plan exists**: do not silently proceed. Surface this explicitly:

> "This sprint has no QA plan. A sprint plan without a QA plan means test requirements are undefined â€?developers won't know what 'done' looks like from a QA perspective, and the sprint cannot pass the Production â†?Polish gate without one.
>
...
- Options:
  - `[A] Run /qa-plan sprint now â€?I'll do that before starting implementation (Recommended)`
  - `[B] Skip for now â€?I understand QA sign-off will be blocked at the Production â†?Polish gate`

If [A]: close with "Sprint plan written. Run `/qa-plan sprint` next â€?then begin implementation."
If [B]: add a warning block to the sprint plan document:

```markdown
> âš ï¸ **No QA Plan**: This sprint was started without a QA plan. Run `/qa-plan sprint`
> before the last story is implemented. The Production â†?Polish gate requires a QA
...
- `/qa-plan sprint` â€?**required before implementation begins** â€?defines test cases per story so developers implement against QA specs, not a blank slate
- `/story-readiness [story-file]` â€?validate a story is ready before starting it
- `/dev-story [story-file]` â€?begin implementing the first story
- `/sprint-status` â€?check progress mid-sprint
- `/scope-check [epic]` â€?verify no scope creep before implementation begins
