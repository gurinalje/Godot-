...
- Omit `partial` keyword on node classes (source generator fails â€?extremely hard to debug)
...
- `godot-gdscript-specialist` when the project uses both languages â€?agree on which system owns which files
