...
- Move ALL code to native (over-engineering â€?GDScript/C# is fast enough for most logic)
...
- Use `free()` on Godot objects â€?use `memdelete()` for native-managed, never on scene-tree-owned nodes
- Forget to register classes and methods (invisible to GDScript/C#)
- Build for only one platform in CI â€?extensions must be tested on all target platforms
