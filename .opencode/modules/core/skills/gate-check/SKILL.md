...
0. **Exploration** â€?Pre-workflow: rapid prototyping multiple ideas (/explore)
1. **Concept** â€?Brainstorming, game concept document
2. **Systems Design** â€?Mapping systems, writing GDDs
3. **Technical Setup** â€?Engine config, architecture decisions
4. **Pre-Production** â€?Prototyping, vertical slice validation
5. **Production** â€?Feature development (Epic/Feature/Task tracking active)
6. **Polish** â€?Performance, playtesting, bug fixing
7. **Release** â€?Launch prep, certification

**Special gate: `workflow-selection`** â€?Only valid from the Exploration stage.
...
- **With argument**: `/gate-check production` â€?validate readiness for that specific phase
  `/gate-check workflow-selection` â€?run workflow selection (only valid from exploration stage)
- **No argument**: Auto-detect current stage using the same heuristics as
  ...
  - Options:
    ...
    - `[B] No â€?pick a different gate` (if selected, show a second widget listing all gate options: Workflow Selection, Concept â†?Systems Design, Systems Design â†?Technical Setup, Technical Setup â†?Pre-Production, Pre-Production â†?Production, Production â†?Polish, Polish â†?Release)

  **Special case â€?exploration stage**: If auto-detect returns `exploration`, the only valid
...
### Gate: Workflow Selection (Exploration â†?Hybrid or Full)

This is a **pre-workflow gate**. It does not validate artifacts â€?it helps the user
...
Skip artifact scanning â€?this gate is human-driven. Use `question` to ask
each question. Present one at a time, tally the score, then recommend. Record
the user's answers but do not write them to a file â€?the output is the
recommendation.

**Question 1 â€?Team size:**
- **Prompt**: "How many people are working on this project?"
- **Options**:
  - `Solo` â€?Just me (score: 0)
  - `Small team` â€?2-5 people (score: 1)
  - `Medium team` â€?6-10 people (score: 2)
  - `Large team` â€?10+ people (score: 3)

**Question 2 â€?Timeline:**
- **Prompt**: "What's your target timeline to a shippable game?"
- **Options**:
  - `Short` â€?Under 3 months (score: 0)
  - `Medium` â€?3-6 months (score: 1)
  - `Long` â€?6-12 months (score: 2)
  - `Extended` â€?12+ months (score: 3)

**Question 3 â€?Design clarity:**
- **Prompt**: "How well-defined is your game design at this point?"
- **Options**:
  - `Rough concept` â€?A theme and a mechanic, but no details (score: 0)
  - `Some systems` â€?Core loop and 2-3 major systems sketched (score: 1)
  - `Mostly designed` â€?Core systems, economy, and progression mapped (score: 2)
  - `Fully documented` â€?Full GDDs ready to build from (score: 3)

**Question 4 â€?External requirements:**
- **Prompt**: "Do you have publisher, investor, or platform requirements?"
- **Options**:
  - `None` â€?Self-funded, no external obligations (score: 0)
  - `Informal` â€?A publisher is interested but no contract yet (score: 1)
  - `Contractual` â€?Signed deal with milestone deliverables (score: 2)
  - `Multi-platform` â€?Strict cert requirements across platforms (score: 3)

**Question 5 â€?Team experience:**
- **Prompt**: "What's your team's experience level with shipping games?"
- **Options**:
  - `First game` â€?No shipped titles (score: 0)
  - `Small games` â€?Shipped jam games or small commercial titles (score: 1)
  - `Experienced` â€?Shipped 1-3 commercial titles (score: 2)
  - `Veteran` â€?Shipped 3+ commercial titles (score: 3)

### Scoring

After all 5 questions, calculate the total score (0-15):

- **Score 0-5**: Recommend **Hybrid workflow** â€?lightweight discovery phase,
  then production with reduced agent roster. Best for small teams, unknown
  designs, and short timelines.
- **Score 6-10**: Recommend **Hybrid with upgrade path** â€?start with Hybrid,
  but be ready to upgrade to Full OCGS as scope grows. Set workflow-mode to
  `hybrid` but note the upgrade triggers.
- **Score 11-15**: Recommend **Full OCGS workflow** â€?formal GDDs, ADRs,
...
**Reasoning**:
- Team size ([answer]) â†?weight toward [workflow]
- Timeline ([answer]) â†?weight toward [workflow]
- Design clarity ([answer]) â†?weight toward [workflow]
- External requirements ([answer]) â†?weight toward [workflow]
- Team experience ([answer]) â†?weight toward [workflow]
...
- **Options**:
  - `Yes â€?proceed with [recommended workflow]`
  - `No â€?I prefer the [other workflow] instead`
  - `Not sure yet â€?I want to explore more ideas first`

### Apply the Decision

When the user confirms (Yes or No), write both files:

1. **`production/workflow-mode.txt`** â€?write `hybrid` or `full` (single line)
2. **`production/stage.txt`** â€?write `Concept` (single line)
...
### Gate: Concept â†?Systems Design

**Required Artifacts:**
- [ ] `game/design/gdd/game-concept.md` exists and has content
- [ ] Game pillars defined (in concept doc or `game/design/gdd/game-pillars.md`)
- [ ] Visual Identity Anchor section exists in `game/design/gdd/game-concept.md` (from brainstorm Phase 4 art-director output)
...
**Required Artifacts:**
- [ ] Systems index exists at `game/design/gdd/systems-index.md` with at least MVP systems enumerated
- [ ] All MVP-tier GDDs exist in `game/design/gdd/` and individually pass `/design-review`
- [ ] A cross-GDD review report exists in `game/design/gdd/` (from `/review-all-gdds`)
...
- [ ] Art bible exists at `design/art/art-bible.md` with at least Sections 1â€? (Visual Identity Foundation)
...
- [ ] Test framework initialized: `game/tests/unit/` and `game/tests/integration/` directories exist
...
### Gate: Pre-Production â†?Production
...
**Quality Checks:**
- [ ] **Core loop fun is validated** â€?playtest data confirms the central mechanic is enjoyable, not just functional. Explicitly check the Vertical Slice playtest report.
...
- [ ] **Vertical Slice is COMPLETE**, not just scoped â€?the build demonstrates the full core loop end-to-end. At least one complete [start â†?challenge â†?resolution] cycle works.
...
- [ ] All core mechanics from GDD are implemented (cross-reference `game/design/gdd/` with `src/`)
- [ ] Main gameplay path is playable end-to-end
- [ ] Test files exist in `game/tests/unit/` and `game/tests/integration/` covering Logic and Integration stories
- [ ] All Logic stories from this sprint have corresponding unit test files in `game/tests/unit/`
...
Use `Glob('game/design/gdd/gdd-cross-review-*.md')` to find the `/review-all-gdds` report.
If no file matches, mark the "cross-GDD review report exists" artifact as **FAIL** and
surface it prominently: "No `/review-all-gdds` report found in `game/design/gdd/`. Run
...
- Compare `game/design/gdd/` documents against `src/` implementations
...
**Skip for Workflow Selection gate**: This gate is pre-workflow â€?no directors
...
simultaneously â€?do not wait for one before starting the next.

**Spawn in parallel:**

1. **`creative-director`** â€?gate **CD-PHASE-GATE** (`.opencode/docs/director-gates.md`)
2. **`technical-director`** â€?gate **TD-PHASE-GATE** (`.opencode/docs/director-gates.md`)
3. **`producer`** â€?gate **PR-PHASE-GATE** (`.opencode/docs/director-gates.md`)
4. **`art-director`** â€?gate **AD-PHASE-GATE** (`.opencode/docs/director-gates.md`)
...
- [x] game/design/gdd/game-concept.md â€?exists, 2.4KB
...
- [ ] Tests â€?FAILED (3 failures in game/tests/unit/)
...
2. **3 test failures** â€?Fix failing tests in game/tests/unit/ before advancing.
...
and scored transparently. No artifact quality checks need verification â€?the
...
Always ask before writing â€?specify both files for workflow-selection:
...
[If hybrid] [A] Run /brainstorm [winning-idea] â€?formalize your concept
             [B] Run /hybrid-prototype [winning-idea] â€?jump straight into prototyping
             [C] Stop here for this session
[If full]   [A] Run /brainstorm [winning-idea] â€?formalize your concept
             [B] Run /setup-engine â€?choose and configure your engine
             [C] Stop here for this session
```
...
```
Gate passed. What would you like to do next?
[A] Start Pre-Production â€?begin prototyping the Vertical Slice
[B] Write more ADRs first â€?run /architecture-decision [next-system]
[C] Stop here for this session
```
...
- **No explore prototypes yet?** â†?`/explore [idea-name]` to prototype ideas before workflow selection
- **Workflow not yet selected?** â†?`/gate-check workflow-selection` to choose Hybrid or Full OCGS
- **Selected Hybrid but no concept?** â†?`/brainstorm [winning-idea]` to formalize, then `/hybrid-prototype` to build
- **Selected Full OCGS but no engine?** â†?`/setup-engine [engine-name]` to configure the engine
...
- **No accessibility requirements doc?** â†?Use `question` to offer to create it now:
  - Prompt: "The gate requires `design/accessibility-requirements.md`. Shall I create it from the template?"
  - Options: `Create it now â€?I'll choose an accessibility tier`, `I'll create it myself`, `Skip for now`
  - If "Create it now": use a second `question` to ask for the tier:
    - Prompt: "Which accessibility tier fits this project?"
    - Options: `Basic â€?remapping + subtitles only (lowest effort)`, `Standard â€?Basic + colorblind modes + scalable UI`, `Comprehensive â€?Standard + motor accessibility + full settings menu`, `Exemplary â€?Comprehensive + external audit + full customization`
...
- **Less than 3 playtest sessions?** â†?Run more playtests before advancing. Use `/playtest-report` to structure findings.
- **No Difficulty Curve doc?** â†?Consider creating one at `design/difficulty-curve.md` before polish
- **No player journey document?** â†?create `design/player-journey.md` using the player journey template
- **Need a quick sprint check?** â†?`/sprint-status` for current sprint progress snapshot
- **Performance unknown?** â†?`/perf-profile`
- **Not localized?** â†?`/localize`
