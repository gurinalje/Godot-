...
context: |
  Static help skill â€?reads production artifacts directly to determine project state and next steps.
...
1. **Read `production/stage.txt`** â€?if it exists and has content, this is the
   ...
   - "Polish" â†?`polish`
   - "Release" â†?`release`
   - "exploration" â†?`exploration` (pre-workflow, not in catalog)
...
2. **If stage.txt is missing**, infer phase from artifacts (most-advanced match wins):
   ...
   - `prototypes/explore/*/REPORT.md` exists â†?`exploration` (pre-workflow, no concept yet)
   - `prototypes/explore/` directory exists (any files) â†?`exploration`
   - Nothing â†?`concept` (fresh project)

3. **If phase is `exploration`**: Return early with exploration-specific guidance.
   This phase is not in the catalog â€?skip Steps 4-8 and go directly to reporting.
   ...
   No workflow is selected yet â€?no GDDs, no architecture, no sprint plans.

   **Exploration prototypes found: [N]**
   [List each with verdict from REPORT.md]

   ### â†?Next up
   **[Explore another idea]** â€?Run `/explore [description]` to prototype a new idea.
   **[Or: Select a workflow]** â€?When you're done exploring, run
   ...
   ### ~ Also available
   - `/gate-check workflow-selection` â€?compare prototypes and choose workflow
   - `/explore [idea]` â€?prototype another idea
   ```
