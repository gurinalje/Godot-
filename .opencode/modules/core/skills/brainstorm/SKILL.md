---
name: brainstorm
description: "Guided game concept ideation â€?from zero idea to a structured game concept document. Uses professional studio ideation techniques, player psychology frameworks, and structured creative exploration."
...
2. **Check for existing concept work**:
   - Read `game/design/gdd/game-concept.md` if it exists (resume, don't restart)
   - Read `game/design/gdd/game-pillars.md` if it exists (build on established pillars)
...
3. **Run through ideation phases** interactively, asking the user questions at
   ...
   **Use `question`** at key decision points throughout brainstorming:
   ...
   - Encourage unusual ideas â€?outside-the-box thinking sparks better concepts
...
- What 3 games have you spent the most time with? What kept you coming back?
  *(Ask this as plain text â€?the user must be able to type specific game names freely.
...
- Do you prefer games that challenge you, relax you, tell you stories,
  or let you express yourself? *(Use `question` for this â€?constrained choice.)*
...
- Tab "Experience" â€?"What kind of experience do you most want players to have?" (Challenge & Mastery / Story & Discovery / Expression & Creativity / Relaxation & Flow)
- Tab "Timeline" â€?"What's your realistic development timeline?" (Weeks / Months / 1-2 years / Multi-year)
- Tab "Dev level" â€?"Where are you in your dev journey?" (First game / Shipped before / Professional background)

Use exactly these tab names â€?do not rename or duplicate them.

**Synthesize** the answers into a **Creative Brief** â€?a 3-5 sentence
...
**CRITICAL**: This MUST be a plain list call â€?no tabs, no form fields. Use exactly this structure:
...
Do NOT use a `tabs` field here. The `tabs` form is for multi-field input only â€?using it here causes an "Invalid tool parameters" error. This is a plain `prompt` + `options` call.

Never pressure toward a choice â€?let them sit with it.
...
The core loop is the beating heart of the game â€?if it isn't fun in
isolation, no amount of content or polish will save the game.

**30-Second Loop** (moment-to-moment):

Ask these as `question` calls â€?derive the options from the chosen concept, don't hardcode them:

1. **Core action feel** â€?prompt: "What's the primary feel of the core action?" Generate 3-4 options that fit the concept's genre and tone, plus a free-text escape (`I'll describe it`).

2. **Key design dimension** â€?identify the most important design variable for this specific concept (e.g., world reactivity, pacing, player agency) and ask about it. Generate options that match the concept. Always include a free-text escape.
...
- Pillars should feel like they create tension with each other â€?if all
...
**Review mode check** â€?apply before spawning CD-PILLARS and AD-CONCEPT-VISUAL:
- `solo` â†?skip both. Note: "CD-PILLARS skipped â€?Solo mode. AD-CONCEPT-VISUAL skipped â€?Solo mode." Proceed to Phase 5.
- `lean` â†?skip both (not PHASE-GATEs). Note: "CD-PILLARS skipped â€?Lean mode. AD-CONCEPT-VISUAL skipped â€?Lean mode." Proceed to Phase 5.
- `full` â†?spawn as normal.

**After pillars and anti-pillars are agreed, spawn BOTH `creative-director` AND `art-director` via Task in parallel before moving to Phase 5. Issue both Task calls simultaneously â€?do not wait for one before starting the other.**

- **`creative-director`** â€?gate **CD-PILLARS** (`.opencode/docs/director-gates.md`)
  Pass: full pillar set with design tests, anti-pillars, core fantasy, unique hook.

- **`art-director`** â€?gate **AD-CONCEPT-VISUAL** (`.opencode/docs/director-gates.md`)
...
- Tab **"Pillars"**: present creative-director feedback. Options mirror the standard CD-PILLARS handling â€?`Lock in as-is` / `Revise [specific pillar]` / `Discuss further`.
...
The user's selected visual anchor (the named direction or their custom description) is stored as the **Visual Identity Anchor** â€?it will be written into the game-concept document and becomes the foundation of the art bible.

If the creative-director returns CONCERNS or REJECT on pillars, resolve pillar issues before asking for the visual anchor selection â€?visual direction should flow from confirmed pillars.
...
- **Target platform**: Use `question` â€?"What platforms are you targeting for this game?"
  Options: `PC (Steam / Epic)` / `Mobile (iOS / Android)` / `Console` / `Web / Browser` / `Multiple platforms`
  Record the answer â€?it directly shapes the engine recommendation and will be passed to `/setup-engine`.
...
- **Engine experience**: Use `question` â€?"Do you already have an engine you work in?"
  Options: `Godot` / `Unity` / `Unreal Engine 5` / `No preference â€?help me decide`
  - If they pick an engine â†?record it as their preference and move on. Do NOT second-guess it.
  - If "No preference" â†?tell them: "Run `/setup-engine` after this session â€?it will walk you through the full decision based on your concept and platform target." Do not make a recommendation here.
...
**Review mode check** â€?apply before spawning TD-FEASIBILITY:
- `solo` â†?skip. Note: "TD-FEASIBILITY skipped â€?Solo mode." Proceed directly to scope tier definition.
- `lean` â†?skip (not a PHASE-GATE). Note: "TD-FEASIBILITY skipped â€?Lean mode." Proceed directly to scope tier definition.
- `full` â†?spawn as normal.
...
**Review mode check** â€?apply before spawning PR-SCOPE:
- `solo` â†?skip. Note: "PR-SCOPE skipped â€?Solo mode." Proceed to document generation.
- `lean` â†?skip (not a PHASE-GATE). Note: "PR-SCOPE skipped â€?Lean mode." Proceed to document generation.
- `full` â†?spawn as normal.
...
4. **Generate the game concept document** using the template at
   ...
   **Include a Visual Identity Anchor section** in the game concept document with:
   ...
   This section is the seed of the art bible â€?it captures the "everything must
   move" decision before it can be forgotten between sessions.

5. Use `question` for write approval:
- Prompt: "Game concept is ready. May I write it to `game/design/gdd/game-concept.md`?"
- Options: `[A] Yes â€?write it` / `[B] Not yet â€?revise a section first`

If [B]: ask which section to revise using `question` with options: `Elevator Pitch` / `Core Fantasy & Unique Hook` / `Pillars` / `Core Loop` / `MVP Definition` / `Scope Tiers` / `Risks` / `Something else â€?I'll describe`

After revising, show the updated section as a diff or clear before/after, then use `question` â€?"Ready to write the updated concept document?"
Options: `[A] Yes â€?write it` / `[B] Revise another section`
...
**Scope consistency rule**: The "Estimated Scope" field in the Core Identity table must match the full-vision timeline from the Scope Tiers section â€?not just say "Large (9+ months)". Write it as "Large (Xâ€“Y months, solo)" or "Large (Xâ€“Y months, team of N)" so the summary table is accurate.

6. **Suggest next steps** (in this order â€?this is the professional studio
   pre-production pipeline). List ALL steps â€?do not abbreviate or truncate:
   1. "Run `/setup-engine` to configure the engine and populate version-aware reference docs"
   2. "Run `/art-bible` to create the visual identity specification â€?do this BEFORE writing GDDs. The art bible gates asset production and shapes technical architecture decisions (rendering, VFX, UI systems)."
   3. "Use `/design-review game/design/gdd/game-concept.md` to validate concept completeness before going downstream"
   4. "Discuss vision with the `creative-director` agent for pillar refinement"
   5. "Decompose the concept into individual systems with `/map-systems` â€?maps dependencies, assigns priorities, and creates the systems index"
   5. "Author per-system GDDs with `/design-system` â€?guided, section-by-section GDD writing for each system identified in step 4"
   6. "Plan the technical architecture with `/create-architecture` â€?produces the master architecture blueprint and Required ADR list"
   7. "Record key architectural decisions with `/architecture-decision (Ã—N)` â€?write one ADR per decision in the Required ADR list from `/create-architecture`"
   8. "Validate readiness to advance with `/gate-check` â€?phase gate before committing to production"
   9. "Prototype the riskiest system with `/prototype [core-mechanic]` â€?validate the core loop before full implementation"
...
Verdict: **COMPLETE** â€?game concept created and handed off for next steps.
...
> **Context is approaching the limit (â‰?0%).** The game concept document is saved
> to `game/design/gdd/game-concept.md`. Open a fresh Claude Code session to continue
> if needed â€?progress is not lost.
...
1. `/setup-engine` â€?configure the engine and populate version-aware reference docs
2. `/art-bible` â€?establish visual identity before writing any GDDs
3. `/map-systems` â€?decompose the concept into individual systems with dependencies
4. `/design-system [first-system]` â€?author per-system GDDs in dependency order
5. `/create-architecture` â€?produce the master architecture blueprint
6. `/gate-check pre-production` â€?validate readiness before committing to production
