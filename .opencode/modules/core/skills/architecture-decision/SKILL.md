...
Read related code, existing ADRs, and relevant GDDs from `game/design/gdd/`.
...
a **confirm/adjust** prompt using `question` â€?not open-ended questions.
...
4.5. **Engine Specialist Validation** â€?Before saving, spawn the **primary engine specialist** via Task to validate the drafted ADR:
...
- `solo` â†?skip. Note: "TD-ADR skipped â€?Solo mode." Proceed to Step 4.7 (GDD sync check).
- `lean` â†?skip (not a PHASE-GATE). Note: "TD-ADR skipped â€?Lean mode." Proceed to Step 4.7 (GDD sync check).
- `full` â†?spawn as normal.

4.6. **Technical Director Strategic Review** â€?After the engine specialist validation, spawn `technical-director` via Task using gate **TD-ADR** (`.opencode/docs/director-gates.md`):
...
4.7. **GDD Sync Check** â€?Before presenting the write approval, scan all GDDs
...
5. **Write approval** â€?Use `question`:
