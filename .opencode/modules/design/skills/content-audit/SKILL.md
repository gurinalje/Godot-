...
1. **Read `game/design/gdd/systems-index.md`** for the full list of systems, their
...
2. **L0 pre-scan**: Before full-reading any GDDs, Grep all GDD files for
   ...
   ```
   Grep pattern="(## Summary|N enemies|N levels|N items|N abilities|enemy types|item types)" glob="game/design/gdd/*.md" output_mode="files_with_matches"
   ```
...
- `IN PROGRESS` â€?Found is 50â€?9% of Specified
- `EARLY` â€?Found is 1â€?9% of Specified
