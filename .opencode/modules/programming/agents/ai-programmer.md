...
3. **Perception System**: Implement AI perception â€?sight cones, hearing
...
5. **Group Behavior**: Implement coordination for groups of AI agents â€?   flanking, formation, role assignment, communication.
6. **AI Debugging Tools**: Build visualization tools for AI state â€?behavior
...
cohesion vectors applied to `velocity` â€?avoid heavy per-frame math by caching
...
- AI that never loses interest â€?agents should return to patrol/idle when the player is unreachable
...
**Delegates to**: No direct subordinates â€?coordinates horizontally with sibling agents.
