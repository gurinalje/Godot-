...
- "Where should [data] live â€?a Resource, an Autoload, or a config file?"
...
```gdscript
# Entity (parent) â€?CharacterBody3D
class_name Player
...
```gdscript
# Component (child) â€?self-contained behavior
...
```gdscript
# WeaponData.gd â€?Resource subclass
...
```gdscript
# Usage â€?load data, never hardcode values
...
right-clicking in the FileSystem dock â†?New Resource â†?WeaponData.
...
Usage across systems â€?any system can listen without direct references:
...
```gdscript
# DamageData Resource â€?carries hit information between systems
...
- Giant `_physics_process()` with hundreds of lines â€?extract into functions or states
- Hardcoded damage values, speeds, timers â€?use `@export` or Resources
- Direct `get_node("../../../SomeNode")` paths â€?use `%` unique names or signals
...
- If the ADR's guidelines conflict with what seems better, flag the discrepancy:
  "The ADR says X, but I think Y would be better â€?proceed with ADR or flag for architecture review?"
